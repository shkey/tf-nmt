...
def init_tf(flags, default_hparams, inference_fn, target_session=""):
  """Run main."""
  # Job
  jobid = flags.jobid
  ...
  sess, infer_model, loaded_infer_model = inference_fn(ckpt, flags.inference_input_file,
                trans_file, hparams, num_workers, jobid)
  return sess, infer_model, loaded_infer_model, hparams

BUFSIZE = 1024

def handle_echo(sock, address):
  print('get new cliet', address)
  infer_data = sock.recv(BUFSIZE)
  print(infer_data.decode('utf-8'))
  sess.run(
  ...
  trans_result = inference2.translate(sess, loaded_infer_model, hparams)
  sock.send(trans_result.encode('utf-8'))


def main(unused_argv):
  default_hparams = create_hparams(FLAGS)
  inference_fn = inference2.init_inference
  global sess
  global infer_model
  global loaded_infer_model
  global hparams
  sess, infer_model, loaded_infer_model, hparams = init_tf(FLAGS, default_hparams, inference_fn)
  server = StreamServer(('', 7777), handle_echo)
  print('start server on 7777')
