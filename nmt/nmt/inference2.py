...
__all__ = ["load_data", "init_inference",
...
def init_inference(ckpt,
  ...
  sess, loaded_infer_model = single_worker_inference(
      infer_model,
      ckpt,
      inference_input_file,
      inference_output_file,
      hparams)
  return sess, infer_model, loaded_infer_model

def translate(sess, loaded_infer_model, hparams):
  trans_result = nmt_utils.decode_and_evaluate(
        "infer",
        loaded_infer_model,
        sess,
        trans_file=None,
...
def single_worker_inference(infer_model,
  ...
  # Read data
  infer_data = ['this is a my first test.']
